...
对于不想花时间学习\verb|tikz|宏包的同学，我们推荐在PowerPoint中绘制好流程图，然后导出pdf格式插入到文档中，学习成本较低，效果也非常好。
...
\autoref{tab:bicap}是张一白提供的一个\textbf{双语标题}和\textbf{在标题中使用脚注}的示例，对标题有特殊需求的同学可以参考此处的源代码：
\begin{table}[H]
  \centering
  \bicaption[三线表格样例]{一个颇为标准的三线表格\footnote{这个例子来自\href{http://www.ctan.org/tex-archive/macros/latex/contrib/booktabs/booktabs.pdf}{《Publication quality tables in LATEX》}(booktabs宏包的文档)。这也是一个在表格中使用脚注的例子，请留意与threeparttable实现的效果有何不同。}}[Table sample]{A standard three-line table\footnote{该表格演示了如何使用bicaption插入双语标题}}
  \label{tab:bicap}
...
\section{多列图片}
由王成欣提供方案及示例代码。如需对两幅或多幅图片进行横向排版，建议使用\verb|subcaption|包里的\verb|subfigure|功能。效果如下：
...
请注意每行subfigure宽度的总和尽量不要超过一个\verb|\textwidth|，否则图像会自动折叠至下一行。
